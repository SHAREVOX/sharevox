...
console.log(`Environment: ${import.meta.env.MODE}, appData: sharevox${suffix}`);

// バージョン0.14より前の設定ファイルの保存場所
const beforeUserDataDir = app.getPath("userData"); // マイグレーション用

// appnameをvoicevoxとしてsetする
app.setName(`sharevox${suffix}`);

// Electronの設定ファイルの保存場所を変更
const fixedUserDataDir = path.join(app.getPath("appData"), `sharevox${suffix}`);
